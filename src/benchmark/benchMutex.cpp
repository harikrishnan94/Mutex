...
struct BMArgs {
  ...
  friend std::ostream &operator<<(std::ostream &out, BMArgs args) {
    out << "Threads = " << args.num_threads
        << ", Critical Section Duration = " << args.crit_section_duration
        << " ns"
...
void bench_worker(MutexType &m, std::atomic<bool> &quit, uint64_t &shared_data,
...
template <typename MutexType> BMResult bench_mutex(BMArgs args) {
  ...
  for (int i = 0; i < args.num_threads; i++) {
    workers.emplace_back(bench_worker<MutexType>, std::ref(m), std::ref(quit),
                         std::ref(shared_data), args.crit_section_duration,
                         args.local_section_duration, std::ref(operations));
  }
  ...
  auto total_operations = std::accumulate(operations.begin(), operations.end(),
                                          0, std::plus<uint64_t>{});
...
static void do_bench(BMArgs args) {
  ...
  if (args.parkinglot && args.pthread) {
    std::cout << std::setw(25) << "Improvement = " << std::fixed
              << std::setprecision(2)
              << (100 * (parkinglot_result.ops /
                             static_cast<double>(pthread_result.ops) -
                         1))
              << "%\n";
  }
}

int main(int argc, const char *argv[]) { do_bench(parse_args(argc, argv)); }
...
static void report_bench(const std::string &str, BMResult result) {
  ...
  if (result.ops) {
    std::cout << std::setw(25) << str << human_readable_num(result.ops)
...
static uint64_t std_dev(const std::vector<uint64_t> &vec) {
  ...
  std::for_each(std::begin(vec), std::end(vec),
                [&](auto val) { variance += (val - mean) * (val - mean); });
...
static BMArgs parse_args(int argc, const char *argv[]) {
  ...
  desc.add_options()("help,h", "Help screen")(
      "numthreads",
      value<int>()
          ->default_value(std::max(std::thread::hardware_concurrency(), 1U))
          ->required(),
      "# Threads")("exectime", value<int>()->default_value(1)->required(),
                   "Execution time of benchmark in seconds")(
      "critsection", value<uint64_t>()->default_value(1)->required(),
      ...
      "critical section (ns)")("localsection",
                               ...
                               "critical section (ns)")(
      "pthread", value<bool>()->default_value(true)->required(),
  ...
  if (vm.count("help")) {
    ...
    try {
      ...
      if (!args.parkinglot && !args.pthread) {
        throw std::string{"ERROR: Must include benchmark for one of Pthread / "
                          "Parkinglot Mutex"};
      }
